...
const KNOWLEDGE_DOCUMENTS: KnowledgeDocument[] = [
  {
    ...
    body: "Use the Empire Knowledge Core as a lightweight retrieval layer for stable business context, operating rules, and curated deployment guidance. Prefer short grounded answers and include only the most relevant snippets in model context.",
  },
  {
    id: "deployment",
    title: "Vercel Deployment Guidance",
    tags: ["vercel", "deployment", "serverless", "runtime", "health"],
    body: "Vercel deployments should avoid depending on persistent local files. Validate required environment variables, expose a health endpoint, and keep agent routes on the Node.js runtime with bounded execution time.",
  },
  {
    id: "wallet-persistence",
    title: "Wallet Persistence",
    tags: ["wallet", "persistence", "secrets", "storage", "env"],
    body: "For serverless environments, prefer storing exported wallet state in a secret such as AGENTKIT_WALLET_DATA instead of relying on wallet_data.txt. File persistence should remain a local-development fallback only.",
  },
  {
    id: "performance",
    title: "Performance Defaults",
    tags: ["performance", "streaming", "cache", "history", "latency"],
    body: "Keep prompts concise, stream responses, bound conversation history, reduce the action-provider surface, and cache read-only tool results with a short TTL to reduce cost and latency.",
  },
  {
    id: "security",
    title: "Security and Secrets",
    tags: ["security", "headers", "secrets", "environment", "safety"],
    body: "Never expose secrets to the client. Configure environment variables in Vercel Project Settings, disable powered-by headers, use restrictive headers for framing and MIME sniffing, and avoid logging sensitive tool payloads in production.",
  },
];
